...
program
    ...
    try {
        ...
        if (combinedOptions.data) {
            ...
            const storageObject = storageObjects.find((so) => so.name === contractName);
            if (!storageAddress)
                throw Error(`Could not find "${contractName}" contract in list of parsed storage objects`);
            await (0, converterClasses2Storage_1.addStorageValues)(combinedOptions.url, storageAddress, storageObject, combinedOptions.blockNumber);
        }
